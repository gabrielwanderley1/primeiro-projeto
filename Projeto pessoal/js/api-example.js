    ...
    db.serialize(async () => {
      ...
      `, (err) => {
        if (err) {
          ...
          db.get("SELECT COUNT(*) as count FROM media_items", async (err, row) => {
            if (err) {
              console.error('Erro ao verificar dados:', err);
            } else if (row.count === 0) {
              await insertSampleData(); // Esperar a inserção dos dados
            }
            resolve();
          });
        }
...
// Função para buscar a URL da imagem (versão do backend)
async function fetchImageUrlForSeed(title, type) {
  if (type === 'movie' && API_CONFIG.TMDB_API_KEY && API_CONFIG.TMDB_API_KEY !== 'SUA_CHAVE_TMDB_AQUI') {
    try {
      const url = `https://api.themoviedb.org/3/search/movie?api_key=${API_CONFIG.TMDB_API_KEY}&query=${encodeURIComponent(title)}`;
      const response = await axios.get(url);
      if (response.data.results && response.data.results.length > 0) {
        return `https://image.tmdb.org/t/p/w500${response.data.results[0].poster_path}`;
      }
    } catch (error) {
      console.error(`Erro ao buscar imagem no TMDb para "${title}":`, error.message);
    }
  } else if (type === 'game' && API_CONFIG.RAWG_API_KEY && API_CONFIG.RAWG_API_KEY !== 'SUA_CHAVE_RAWG_AQUI') {
    try {
      const url = `https://api.rawg.io/api/games?key=${API_CONFIG.RAWG_API_KEY}&search=${encodeURIComponent(title)}`;
      const response = await axios.get(url);
      if (response.data.results && response.data.results.length > 0) {
        return response.data.results[0].background_image;
      }
    } catch (error) {
      console.error(`Erro ao buscar imagem no RAWG para "${title}":`, error.message);
    }
  }
  return null;
}
...
async function insertSampleData() {
  ...
  const sampleItems = [
    { type: 'movie', title: 'Inception', year: '2010', genre: 'Ficção Científica', rating: 5, status: 'completed', favorite: 1 },
    { type: 'game', title: 'The Witcher 3', year: '2015', genre: 'RPG', rating: 5, status: 'completed', favorite: 1 },
    { type: 'movie', title: 'Interstellar', year: '2014', genre: 'Drama, Ficção Científica', rating: 4, status: 'watchlist', favorite: 0 },
    { type: 'game', title: 'Cyberpunk 2077', year: '2020', genre: 'RPG, Ação', rating: 4, status: 'completed', favorite: 0 }
  ];
  ...
  for (const item of sampleItems) {
    const imageUrl = await fetchImageUrlForSeed(item.title, item.type);
    console.log(`🖼️ Imagem para "${item.title}": ${imageUrl || 'Não encontrada'}`);
